...
//edit by fae
/* VIDL - Product ID Low Byte Register */
#define DAVICOM_VIDL		0x28
...
//edit by fae
/* RXBCR - RX Broadcast Control Register */
#define DM9051_RXBCR            0x53
/* MPCR - Memory Pointer Control Register */
...
//edit by fae
#define DM9051_RXBCR_EN		    0xC0

#define DM9051_MPCR_RST_TX		BIT(1)
#define DM9051_MPCR_RST_RX		BIT(0)
...
#define ETH_DM9051_MULTICAST_FILTER
...
#ifdef ETH_DM9051_MULTICAST_FILTER
    uint8_t multcast_hashtable[64];
#endif
};
...
{
//edit by fae
//Adding a VID check allows for compatibility with more products.
	uint16_t vid;
	uint16_t pid;
	int ret;

	/* Read Vendor ID */
	ret = eth_dm9051_spi_read_regs(dev, DAVICOM_VIDL, (void *)&vid, 2);
	if (ret < 0) {
		return ret;
	}
	...
	/* Check product ID */
	if ((sys_le16_to_cpu(pid) != DM9051_ID) || (sys_le16_to_cpu(vid) != DAVICOM_ID)) {
		LOG_ERR("%s: Found ID: %04x%04x, Expected ID: %04x%04x\n", dev->name, vid, pid, DAVICOM_ID, DM9051_ID);
		return -EIO;
	}
...
#ifdef ETH_DM9051_MULTICAST_FILTER
static uint32_t crc32_le(uint8_t *data, size_t length)
{
...
static void dm9051_modify_multicastaddr(const struct device *dev, uint8_t *multicastaddr,
                    bool action)
{
    struct  eth_dm9051_data *ctx = dev->data;
    uint8_t hash_value;
    uint8_t hash_group;
    uint8_t hash_bit;
    ...
    hash_value = crc32_le(multicastaddr, 6) & 0x3F;

    hash_group = hash_value / 8;
    hash_bit = hash_value % 8;

    LOG_DBG("%s: dm9051_modify_multicastaddr ht_%02x , mar %02x , bit %d", dev->name,
        hash_value, DM9051_MAR + hash_group, hash_bit);

    eth_dm9051_spi_read_reg(dev, (DM9051_MAR + hash_group), &modify_mar);

    if (action) {
        ctx->multcast_hashtable[hash_value]++;
        modify_mar |= (1 << hash_bit);

        LOG_INF("%s: dm9051_modify_multicastaddr add [%02X:%02X:%02X:%02X:%02X:%02X]",
            dev->name, multicastaddr[0], multicastaddr[1], multicastaddr[2],
            multicastaddr[3], multicastaddr[4], multicastaddr[5]);
    } else {
        if (ctx->multcast_hashtable[hash_value] > 0) {
            ctx->multcast_hashtable[hash_value]--;
        }

        if (0 == ctx->multcast_hashtable[hash_value]) {
            modify_mar &= ~(1 << hash_bit);
        }

        LOG_INF("%s: dm9051_modify_multicastaddr remove [%02X:%02X:%02X:%02X:%02X:%02X]",
            dev->name, multicastaddr[0], multicastaddr[1], multicastaddr[2],
            multicastaddr[3], multicastaddr[4], multicastaddr[5]);
    }

    eth_dm9051_spi_write_reg(dev, (DM9051_MAR + hash_group), modify_mar);
}
#endif
...
//edit by fae
// DM9051_GPR bit 0 is write only , can't read .
#if 1
...
//edti by fae
// MAR is a multicast table setting. If it is not cleared, multicast packets will not be received correctly.
#if 1
#ifdef ETH_DM9051_MULTICAST_FILTER
{
    struct eth_dm9051_data *ctx = dev->data;

    memset(ctx->multcast_hashtable, 0, 65);
}
#endif

	/* clear Multicast table broadcast packets */
	for(i = 0; i < 8; i++)
	{
		ret = eth_dm9051_spi_write_reg(dev, DM9051_MAR + i, 0x00);
		if (ret < 0) {
			return ret;
		}
	}
	/* Enable broadcast packets */
	ret = eth_dm9051_spi_write_reg(dev, DM9051_RXBCR, DM9051_RXBCR_EN);
	if (ret < 0) {
		return ret;
	}
#else
...
//edit by fae
// The par bit in the imr register must be set before the rcr register.
	/* Enable RX */
	ret = eth_dm9051_spi_write_reg(dev, DM9051_IMR, DM9051_IMR_PAR);
	if (ret < 0) {
		return ret;
	}
...
#if 1
//		reset rx fifo
		ret = eth_dm9051_spi_write_reg(dev, DM9051_MPCR, DM9051_MPCR_RST_RX);
		if (ret < 0) {
			return NULL;
		}
#else
...
//      check rec ready
		...
		if(flag != DM9051_FLAG_RX_PKT)
		{
			...
			{
				// RX data is not available in RX SRAM */
			}
			break;
		}
#else
...
//edit byf fae
    ...
    case ETHERNET_CONFIG_TYPE_FILTER: {
        LOG_DBG("%s: dm9051_set_config ETHERNET_CONFIG_TYPE_FILTER", dev->name);
        dm9051_modify_multicastaddr(dev, (uint8_t *)config->filter.mac_address.addr,
                        config->filter.set);
        return 0;
    }
#endif
